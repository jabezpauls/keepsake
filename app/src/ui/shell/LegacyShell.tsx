// Legacy top-nav shell — preserved during the Phase 2 → Phase 9
// migration so we can A/B compare against the new sidebar shell with
// the `?legacy=1` URL flag.
//
// This file is verbatim the pre-redesign Shell.tsx. Once Phase 9 is
// done and the new shell has covered every screen, this file gets
// deleted in one of the cleanup commits.
    ...
    return (
        <div className="shell">
            ...
            <section className="view-host">
                ...
                {view.kind === "library" && <Timeline />}
                {view.kind === "for-you" && <Memories />}
...
function MlBadge({
    ...
    if (!status.models_available) {
        return (
            <span
                className="ml-badge ml-badge--off"
                title="Build without --features ml-models"
            >
                ML off
            </span>
        );
