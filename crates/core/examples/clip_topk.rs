...
fn main() {
    ...
    // Hit count = how many captions landed their ground-truth id in top-5.
    let mut hits = 0usize;
    for (ground_truth, caption) in &labels {
        ...
        // Search requires a CollectionKey to decrypt device/lens strings
        // when those filters are set; we don't use them here, so pass None.
        // A real eval should pick the vault's default key; we stay
        // conservative and accept the filtered-result ordering.
        ...
        if top5.contains(ground_truth) {
            ...
            eprintln!(
                "miss: gt={ground_truth} caption={caption:?} top5={:?}",
                top5
            );
        }
    }
    ...
    // Also emit a machine-parseable last line for recording.
    let _ = db::count_ml_jobs_by_state; // keep import live without unused warning.
}
